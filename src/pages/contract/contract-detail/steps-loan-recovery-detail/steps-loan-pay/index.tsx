...
import { DeductionFree, loanPayModel } from 'models/contract-detail.model';
import { ItemPropsModel } from 'models/item-props-model';
import IcSelect from 'assets/icon/ic_select.svg';
import { dataCustomerResource, infoPayment, loanPayItemInputRef, radioForm, radioMethod } from 'pages/__mocks__/ContractInfomation';
import React, { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import styles from './loan-pay.module.scss';
...
const LoanPay = () => {
    ...
    const [data, setData] = useState<loanPayModel>({});
    const [toggle, setToggle] = useState<boolean>(false);

    useEffect(() => {
        setData(last => {
            Object.keys(infoPayment).map((key) => {
                last[key] = infoPayment[key];
            });
            setToggle(!toggle);
            return last;
        });
            ...
            <Box sx={{ flexGrow: 1 }} className={cx('box-container')}>
                <Grid container spacing={2} xs={12}>
                    <Grid item xs={12} md={4}>
                        ...
                        {renderInput(loanPayInputRef[7], data?.total_amount_deduction_fee || '', TYPE_INPUT.TEXT, Languages.contract.totalAmountDeductionFee, Languages.contract.import, false)}
                    </Grid>
                    <Grid item xs={12} md={4}>
                        <Grid item xs={12} md={12} className={cx('container-item')} style={{ marginTop: '10px' }}>
                            {renderRadioButtonGroup(loanPayRadioRef[0], radioForm, Languages.contract.form, data?.form || radioForm[0].value, 'title-radio-form', 'form', 5)}
                        </Grid>
                        {renderInput(loanPayInputRef[8], data?.total_amount_due || '', TYPE_INPUT.TEXT, Languages.contract.totalAmountDue, Languages.contract.import, false)}
                        {renderInput(loanPayInputRef[9], data?.money_pay_date || '', TYPE_INPUT.TEXT, Languages.contract.moneyPayDate, Languages.contract.import, false, true)}
                        {renderInput(loanPayInputRef[10], data?.difference || '', TYPE_INPUT.TEXT, Languages.contract.actualDifferenceAndPayout, Languages.contract.import, false)}
                        {renderInput(loanPayInputRef[11], data?.total_amount_minus || '', TYPE_INPUT.TEXT, Languages.contract.totalAmountMinus, Languages.contract.import, false)}
                        {renderInput(loanPayInputRef[12], data?.forfeit || '', TYPE_INPUT.TEXT, Languages.contract.penaltyForLatePayment, Languages.contract.import, false)}
                        {renderInput(loanPayInputRef[13], data?.overdue_money || '', TYPE_INPUT.TEXT, Languages.contract.overdueMoney, Languages.contract.import, false)}
                    </Grid>
                    <Grid item xs={12} md={4}>
                        <Grid item xs={12} md={12} className={cx('container-item')} style={{ marginTop: '10px' }}>
    ...
    }, [toggle, data?.content, data?.customer_resources, data?.date_payment, data?.deduction_fee, data?.difference, data?.forfeit, data?.form, data?.method, data?.money_paid_by_customer, data?.money_pay_date, data?.number_days_difference, data?.overdue_money, data?.payer_name, data?.payer_phone, data?.reality_date, data?.relationship, data?.total_amount_deduction_fee, data?.total_amount_due, data?.total_amount_minus, data?.valid_money, loanPayInputRef, loanPayRadioRef, renderInput, renderListItem, renderRadioButtonGroup, renderSelect, renderTextArea]);
